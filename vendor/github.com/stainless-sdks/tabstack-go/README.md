...
```go
import (
	"github.com/stainless-sdks/tabstack-go" // imported as tabstack
)
```
...
The tabstack library uses the [`omitzero`](https://tip.golang.org/doc/go1.24#encodingjsonpkgencodingjson)
semantics from the Go 1.24+ `encoding/json` release for request fields.

Required primitive fields (`int64`, `string`, etc.) feature the tag <code>\`api:"required"\`</code>. These
fields are always serialized, even their zero values.

Optional primitive types are wrapped in a `param.Opt[T]`. These fields can be set with the provided constructors, `tabstack.String(string)`, `tabstack.Int(int64)`, etc.
...
```go
p := tabstack.ExampleParams{
	ID:   "id_xxx",               // required property
	Name: tabstack.String("..."), // optional property

	Point: tabstack.Point{
		X: 0,               // required field will serialize as 0
		Y: tabstack.Int(1), // optional field will serialize as 1
		// ... omitted non-required fields will not be serialized
	},

	Origin: tabstack.Origin{}, // the zero value of [Origin] is considered omitted
}
```
...
custom := param.Override[tabstack.FooParams](12)
```
...
See the [full list of request options](https://pkg.go.dev/github.com/stainless-sdks/tabstack-go/option).
...
`*tabstack.Error`. This contains the `StatusCode`, `*http.Request`, and
...
We also provide a helper `tabstack.File(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ...
    Data: FooNewParamsData{
        FirstName: tabstack.String("John"),
    },
}
